...
//            emitEvent(CheckoutEvent.NavigateToOrderTracking)
    ...
    private fun pollMpesaStatus(checkoutRequestId: String, orderId: String) {
        launch {
            var attempts = 0
            val maxAttempts = 6 // Poll for ~30 seconds (5s * 6)
            var isSuccess = false
            ...
            if (isSuccess) {
                ...
                notificationHelper.showNotification(
                    ...
                    message = "We couldn\u0027t verify your M-Pesa payment yet. It might still be processing.",
                    orderId = orderId
                )
                setState { it.copy(error = "Payment verification timed out. Check order history.") }
            }
