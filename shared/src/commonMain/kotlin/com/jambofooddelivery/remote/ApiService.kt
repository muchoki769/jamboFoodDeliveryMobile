...
import com.jambofooddelivery.models.AuthResponse
...
import com.jambofooddelivery.models.PaymentResponse
...
import com.jambofooddelivery.models.MenuCategory
...
interface ApiService {
    ...
    // Paystack routes
    suspend fun initiatePaystackPayment(request: PaystackInitiateRequest): ApiResponse<PaystackInitiateResponse>
    suspend fun verifyPaystackPayment(reference: String): ApiResponse<Boolean>
}
...
class ApiServiceImpl(private val client: HttpClient) : ApiService {
    ...
    override suspend fun initiatePaystackPayment(request: PaystackInitiateRequest): ApiResponse<PaystackInitiateResponse> = safeRequest {
        client.post("payments/paystack/initiate") {
            contentType(ContentType.Application.Json)
            setBody(request)
        }
    }

    override suspend fun verifyPaystackPayment(reference: String): ApiResponse<Boolean> = safeRequest {
        client.post("payments/paystack/verify") {
            contentType(ContentType.Application.Json)
            setBody(mapOf("reference" to reference))
        }
    }
}
...
@Serializable
data class PaystackInitiateResponse(
